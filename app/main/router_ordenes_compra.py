...
@main.route('/compras/ordenes-compra')
@login_required
def compras_ordenes_compra():
    oc_rows   = db.session.execute(text("SELECT * FROM sp_oc_listar(0)")).fetchall()
    item_rows = db.session.execute(text("SELECT * FROM sp_oc_items_listar(0)")).fetchall()

    items_by_oc = {}
    for r in item_rows:
        d = _item_to_dict(r)
        items_by_oc.setdefault(d['oc_id'], []).append(d)

    oc_list = []
    for r in oc_rows:
        d = _row_to_dict(r)
        d['items'] = items_by_oc.get(d['oc_id'], [])
        oc_list.append(d)
    ...
    return render_template('main/ordenes_compra.html',
                           title='Órdenes de Compra',
                           section='Compras', page='Órdenes de Compra',
                           oc_json=json.dumps(oc_list, ensure_ascii=False),
                           total=len(oc_list),
                           usuarios=usuarios,
...
@main.route('/compras/ordenes-compra/nueva', methods=['GET', 'POST'])
@login_required
def compras_ordenes_compra_nueva():
    if request.method == 'GET':
        today = date.today().isoformat()
        return render_template('main/ordenes_compra_nueva.html',
                               title='Nueva Orden de Compra',
                               section='Compras', page='Nueva OC',
                               today=today)
    ...
    if not card_code:
        flash('El Proveedor es obligatorio.', 'danger')
        return redirect(url_for('main.compras_ordenes_compra_nueva'))
    if not items_list:
    ...
    try:
        ...
        if row and row[0]:
            db.session.commit()
            flash(f'Orden de Compra OC-{row[2]:05d} registrada correctamente.', 'success')
            return redirect(url_for('main.compras_ordenes_compra'))
        else:
            db.session.rollback()
            flash(row[1] if row else 'Error al registrar la Orden de Compra.', 'danger')
...
@main.route('/compras/ordenes-compra/<int:oc_id>/anular', methods=['POST'])
@login_required
def compras_orden_compra_anular(oc_id):
    try:
        row = db.session.execute(
            text("SELECT success, message FROM fn_oc_anular(:id)"),
            {'id': oc_id}
        ).fetchone()
        if row and row[0]:
            db.session.commit()
            flash(f'OC-{oc_id:05d} anulada correctamente.', 'success')
        else:
            db.session.rollback()
...
# ── API: ítems para OC (todos los ítems con avg_price) ───────

@main.route('/api/items-compras')
@login_required
def api_items_compras():
    rows = db.session.execute(text("""
        SELECT
            item_code,
            item_name,
            COALESCE(avg_price, 0)                         AS avg_price,
            COALESCE(buy_unit_msr, sal_unit_msr, '')       AS unit_msr,
            COALESCE(tax_code_ap, 'I18')                   AS tax_code
        FROM items
